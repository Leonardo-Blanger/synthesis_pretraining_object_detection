...
parser.add_argument('--output_dir', default='../data/qr_codes_fake')
...
def sample_object():
    ...
    text_len = np.random.randint(1, 101)
    letters = string.ascii_lowercase
    text = ''.join(random.choice(letters) for i in range(text_len))
    qr.add_data(text)
    qr.make(fit=True)

    obj = qr.make_image(fill_color="black", back_color="white")
    obj = np.array(obj).astype(np.uint8) * 255
    obj = np.stack([obj, obj, obj], axis=2)

    return obj


def calculate_mask(obj):
    block_mask = (obj < 255).any(axis=2)
    row_mask = block_mask.any(axis=1)
    col_mask = block_mask.any(axis=0)
    ...
    mask = np.zeros(block_mask.shape, dtype=np.float32)
