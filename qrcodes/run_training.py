...
import config
from qr_codes_loader import load_qr_codes, build_dataset
...
trainval_image_paths, trainval_bnd_boxes = load_qr_codes(split='train')
(train_image_paths, valid_image_paths,
 train_bnd_boxes, valid_bnd_boxes) = train_test_split_tensors(
     trainval_image_paths, trainval_bnd_boxes, test_size=config.VALID_SAMPLES,
...
for run in range(1, config.NUM_RUNS+1):
    ...
    for architecture in config.ARCHITECTURES:
        ...
        if not os.path.exists(model_path):
            ...
            fake_image_paths, fake_bnd_boxes = load_qr_codes(
                root=os.path.join('..', 'data', 'qr_codes_fake'),
                split='fake')
