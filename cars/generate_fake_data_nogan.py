...
parser.add_argument('--output_dir', default='../data/cars_fake_nogan')
...
parser.add_argument('--max_scale', default=1.0, type=float,
                    ...
                    to the image size.')
parser.add_argument('--min_aspect_ratio', default=0.333, type=float,
                    help='Minimum aspect ratio for the boxes (w / h).')
parser.add_argument('--max_aspect_ratio', default=3.0, type=float,
...
parser.add_argument('--real_objects_dir', default='../data/LSUNCars', type=str,
...
fake_image_paths = [os.path.join(REAL_OBJECTS_DIR, img_file)
                    for img_file in os.listdir(REAL_OBJECTS_DIR)]

def sample_object():
    image_path = np.random.choice(fake_image_paths)
    image = Image.open(image_path)
    image = image.resize((2*MASK_SIZE, 2*MASK_SIZE))
    r0, c0 = MASK_SIZE//2, MASK_SIZE//2
    r1, c1 = r0+MASK_SIZE, c0+MASK_SIZE
    image = image.crop((r0, c0, r1, c1))
    ...
    while True:
        ...
        for _ in range(MAX_TRIES):
            ...
            for _ in range(num_objects):
                car_size = np.random.randint(MIN_SIZE, MAX_SIZE)
                car_ar = np.random.uniform(MIN_ASPECT_RATIO, MAX_ASPECT_RATIO)
                car_width = min(IMAGE_SIZE, int(car_size * np.sqrt(car_ar)))
                car_height = min(IMAGE_SIZE, int(car_size / np.sqrt(car_ar)))

                y1 = np.random.randint(0, IMAGE_SIZE - car_height + 1)
                x1 = np.random.randint(0, IMAGE_SIZE - car_width + 1)
                y2 = y1 + car_height
                x2 = x1 + car_width
                bnd_boxes.append(
                    ia.BoundingBox(x1=x1, y1=y1, x2=x2, y2=y2, label='car')
                )
...
ANNOTATIONS_FILE = os.path.join(OUTPUT_DIR, 'cars_fake.csv')
