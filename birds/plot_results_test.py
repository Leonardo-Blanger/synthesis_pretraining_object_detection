...
for architecture in config.ARCHITECTURES:
    ...
    for train_type in ['from_scratch', 'finetuned']:
        ...
        for num_samples, mAP, std in zip(config.TRAIN_SAMPLES, avg_meanAP, std_meanAP):
            print("\n{} {} on {} samples ({:.0f}): {:.2f}% +- {:.2f}%".format(architecture, train_type, num_samples, 100*num_samples/ALL_SAMPLES, mAP, std))
    ...
    plt.savefig(architecture.lower() + '_test_results_birds.png')
    plt.show()
