...
for architecture in config.ARCHITECTURES:
    ...
    for num_samples in config.TRAIN_SAMPLES:
        if (num_samples == 10 or num_samples%100 == 0 or num_samples == ALL_SAMPLES) and num_samples not in (100, 300):
    ...
    plt.ylim(ymax = 100)
    plt.title(architecture + " (Faces)", size=20)
    ...
    plt.savefig(architecture.lower() + '_test_results_faces.png')
    plt.show()
